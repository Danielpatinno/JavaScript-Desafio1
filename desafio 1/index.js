...
class user {
  ...
  getmes() {
    ...
    return (
      "Ultima transação realizada dia " +
      date.getDate() +
      " do mes de " +
      month +
      " de " +
...
class bank extends user {
  ...
  userlist() {
    ...
    switch (actionList) {
      case "1":
        ...
        break;
      case "2":
        var indexUser = prompt(
          "Qual usuario você deseja acessar ?\n0 - Daniel\n1 - Fidel\n2 - Daniele\n3 - Jose Luis"
        );
  ...
  createUser() {
    ...
    const usercreat = new user(
      prompt("Por favot, Dígite um nome para o novo usuario"),
      this.promptNum(
        "Número de identidade do cliente",
        "Você deve digitar um número \nPor favor, Tente novamente"
      ),
      prompt("Dígite a data de aniversário do cliente"),
      prompt("Nome da Rua do cliente"),
      this.promptNum(
        "Número da casa do cliente",
        "Você deve digitar um número \nPor favor, Tente novamente"
      ),
      this.promptNum("CEP do novo usuario"),
      ""
    );
  ...
  deposit() {
    ...
    users[customerDeposit].balanceCurrent =
      users[customerDeposit].balanceCurrent + deposito;
    users[customerDeposit].transactions.transactions =
      "Você depositou " + deposito;
    users[customerDeposit].transactions.dateLastTransactions = this.getmes();
    alert(
      "Você efetuou um deposito\nSeu saldo atual é " +
        users[customerDeposit].balanceCurrent
    );
  ...
  consult() {
    ...
    alert(
      this.getNameUser(query) + " o seu saldo é " + users[query].balanceCurrent
    );
